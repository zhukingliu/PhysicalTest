					...
					<!-- Row start -->
						<div class="am-u-sm-12">
							<div class="card-box">
								...
								<form action="doUpdateStudent.jsp" method="post" class="am-form" data-am-validator>
									<fieldset>
										<legend>修改学生信息</legend>
										<div class="am-form-group">
											<label>学号：</label>
											<input type="text" name="Id" value="<%=student.getStudentID() %>" required/>
										</div>
										<div class="am-form-group">
											<label>姓名：</label>
											<input type="text" name="Name" value="<%=student.getName() %>" placeholder="输入姓名" required/>
										</div>
										<div class="am-form-group">
											<label>出生年月：</label>
											<input type="date" name="birthday" value="<%=student.getBirthday() %>" required/>
										</div>

                                       <div class="am-form-group">
										    <label>性别：</label>
											<select name="sex" value="<%=student.getSex() %>" required>
											<%if(student.getSex()==0){ %>
												<option selected="true">男</option>
												<option>女</option>
												<%}else{ %>
												<option>男</option>
												<option selected="true">女</option><%} %>
											</select>
										</div>

                                        <div class="am-form-group">
											<label>学院：</label>
											<input type="text" name="college" value="<%=student.getCollege() %>" placeholder="所在学院" required/>
										</div>
										<div class="am-form-group">
											<label>专业：</label>
											<input type="text" name="project" value="<%=student.getProject() %>" placeholder="就读专业" required/>
										</div>
										<div class="am-form-group">
											<label>班主任：</label>
											<input type="text" name="teacher" value="<%=teacher.getName() %>" placeholder="班主任" required/>
										</div>
										<div class="am-form-group">
											<label>领导：</label>
											<input type="text" name="leader" value="<%=leader.getLeaderName() %>" placeholder="领导" required/>
										</div>
